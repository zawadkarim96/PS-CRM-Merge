...
LETTERHEAD_FILENAME = "ps_letterhead.png"


def preferred_letterhead_location() -> Path:
    """Return the location where the letterhead should reside."""

    target = CONFIG.data_dir / "letterhead" / LETTERHEAD_FILENAME
    target.parent.mkdir(parents=True, exist_ok=True)
    return target


def resolve_letterhead_path() -> Optional[Path]:
    """Locate the letterhead, favoring the writable storage directory."""

    storage_path = preferred_letterhead_location()
    if storage_path.exists():
        return storage_path

    repo_fallback = Path(__file__).with_name(LETTERHEAD_FILENAME)
...
def load_letterhead_base64() -> Optional[str]:
    ...
    letterhead_path = resolve_letterhead_path()
...
def render_letter_preview(state: Dict[str, Any]) -> None:
    ...
    if LETTERHEAD_BASE64:
        ...
        expected_path = preferred_letterhead_location()
        st.warning(
            "Letterhead image missing. "
            f"Place {LETTERHEAD_FILENAME} at {expected_path} to enable preview."
        )
...
def init_db() -> None:
    with get_cursor() as cur:
        ...
        cur.execute(f"PRAGMA table_info({DELIVERY_TABLE})")
...
def _ensure_user_dict(raw_user: Dict) -> Dict:
    return {
        "user_id": raw_user.get("user_id"),
        "username": raw_user.get("username"),
        "role": raw_user.get("role", "staff"),
        "display_name": raw_user.get("display_name") or raw_user.get("username"),
        "designation": raw_user.get("designation"),
        "phone": raw_user.get("phone"),
    }


def _render_sales_pages(user: Dict) -> None:
    page = sidebar(user)
...
def render_embedded(user: Dict | None) -> None:
    init_db()
    if not user:
        st.info("Please log in to access sales features.")
        return
    session_user = _ensure_user_dict(user)
    st.session_state["user"] = session_user
    _render_sales_pages(session_user)
...
def main() -> None:
    ...
    user = _ensure_user_dict(st.session_state["user"])
    st.session_state["user"] = user
    _render_sales_pages(user)
