...
except ModuleNotFoundError:  # pragma: no cover - defensive for embedded imports
    from pathlib import Path as _Path

    def get_storage_dir() -> _Path:
        return _Path(os.getenv("APP_STORAGE_DIR", _Path.home()))
...
def load_config() -> AppConfig:
    ...
    if not db_url:
        default_db = Path(os.environ.get("DB_PATH", data_dir / "ps_crm.db"))
        db_path = default_db.expanduser()
        db_url = (
            f"sqlite:///{db_path}"
            if os.name != "nt"
...
def _default_data_dir() -> Path:
    ...
    base_dir = Path(os.environ.get("APP_STORAGE_DIR", get_storage_dir()))
    return base_dir
